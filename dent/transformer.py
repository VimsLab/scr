...
class Encoder(nn.Module):
	def __init__(self, hidden_dim=256, nheads=8, num_encoder_layers=6, dropout_rate=0.1):
		...
		encoder_layer = nn.TransformerEncoderLayer(d_model=hidden_dim, nhead=nheads, dropout=dropout_rate, batch_first=True)
	...
	def forward(self, inputs):
		...
		encoder_outputs = self.encoder(encoder_embedding) # shape: (batch_size, seq_len, hidden_dim)
